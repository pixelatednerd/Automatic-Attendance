...
class App:    
    def __init__(self,window,video_source=0):
        ...
        frame = tk.Frame(window, width=1280, height=720)
        frame.pack()
    ...
    def get_frame(self, ret=None):
        ...
        if self.vid.isOpened():
            ...
            for(x,y,w,h) in faces:
                ...
                if prediction[1]<80:
                    cv2.putText(frame,'%s-(%.2f)%%'%(names[prediction[0]],prediction[1]),(x-10,y-10),cv2.FONT_HERSHEY_DUPLEX,2,(255,255,255),2)
                    self.mark_student(names[prediction[0]])
                else:
                    cv2.putText(frame,"Unknown Student",(x-10,y-10),cv2.FONT_HERSHEY_DUPLEX,2,(255,255,255),2)
...
class ElapsedTimeClock:
    def __init__(self,window):
        self.T=tk.Label(window,text="00:00:00",font=('times',20,'bold'),bg="burlywood2",fg="purple")
